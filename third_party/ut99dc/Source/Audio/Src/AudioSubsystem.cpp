...
void UGenericAudioSubsystem::RegisterMusic( UMusic* Music )
{
	...
	if( !Music->Handle )
	{
		// Set a temporary handle to avoid re-entrant loads while the package data is resident.
		Music->Handle = (void*)-1;

		Music->Data.Load();
		debugf( NAME_DevMusic, TEXT("Register music: %s (%i)"), Music->GetFullName(), Music->Data.Num() );

		if( Music->Data.Num() > 0 )
		{
			// Keep the original UMX payload immutable.  Some old module readers expect
			// padding, so copy to a temporary padded block instead of AddZeroed() on
			// the lazy array itself.
			INT PaddedSize = Music->Data.Num() + 1024;
			BYTE* PaddedData = (BYTE*)appMalloc( PaddedSize, TEXT("Generic music module data") );
			appMemcpy( PaddedData, &Music->Data(0), Music->Data.Num() );
			appMemset( PaddedData + Music->Data.Num(), 0, 1024 );

			MemChunk MusicChunk;
			MusicChunk.Data       = PaddedData;
			MusicChunk.DataLength = PaddedSize;
			MusicChunk.Position   = 0;

			if( !LoadMusic( &MusicChunk, Music->GetFullName() ) )
			{
				debugf( NAME_Warning, TEXT("Could not load digital music: %s"), Music->GetFullName() );
				Music->Handle = NULL;
			}
		...
		{
			debugf( NAME_Warning, TEXT("Could not load digital music: %s has no module data"), Music->GetFullName() );
			Music->Handle = NULL;
		}
...
//					Viewport->GetOuterUClient()->Engine->Render->GlobalLighting( (Viewport->Actor->ShowFlags & SHOW_PlayerCtrl)!=0, Playing.Actor, Brightness, Color );
	...
	// Unlock before touching UMusic lazy payloads. RegisterMusic/LoadMusic use the
	// same audio mutex internally, and the old Android SDL2_mixer bridge did not
	// depend on UE1 music transition state.  Keep that robust behaviour: follow
	// Viewport->Actor->Song every frame and start/stop by section.
	AUnlock;

	guard(UpdateMusic);
	if( UseDigitalMusic && Viewport && Viewport->Actor )
	{
		UMusic* WantedMusic = Viewport->Actor->Song;
		BYTE WantedSection = Viewport->Actor->SongSection;
		...
		if( WantedMusic != CurrentMusic )
		{
			...
			if( CurrentMusic )
			{
				RegisterMusic( CurrentMusic );
				SetVolumes();
			}
		}

		if( CurrentMusic && CurrentMusic->Handle )
		{
			if( WantedSection == 255 )
			{
				if( CurrentSection != 255 )
					StopMusic();
				CurrentSection = 255;
			}
			else if( WantedSection != CurrentSection || !IsMusicPlaying() )
			{
				UBOOL MusicOk = 0;
				if( CurrentSection == 255 || !IsMusicPlaying() )
					MusicOk = StartMusic( WantedSection );
				else
					MusicOk = SetMusicPosition( WantedSection );

				CurrentSection = MusicOk ? WantedSection : 255;
			}
		}
	}
