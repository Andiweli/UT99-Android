...
void UClass::Bind()
{
	...
	if( !ClassConstructor && (GetFlags() & RF_Native) )
	{
		...
		else if( !GIsEditor )
		{
			        ...
			        // UT99_ANDROID_UWEB_BIND_GUARD_V51:
			        // Compiled UT menu/web classes may reference native UWeb.* classes,
			        // but this Android port does not ship/register the UWeb native module.
			        // Do not fatal-error on UWeb; bind to superclass constructor instead.
			        if( GetOuter() && GetOuter()->GetFName()==FName(TEXT("UWeb")) )
			        {
			            UClass* AndroidUWebSuper = GetSuperClass();
			            if( AndroidUWebSuper )
			            {
			                AndroidUWebSuper->Bind();
			                ClassConstructor = AndroidUWebSuper->ClassConstructor;
			            }
			            return;
			        }
			#endif /* UT99_ANDROID_UWEB_BIND_GUARD_V51 */
...
#if defined(__ANDROID__) || defined(PLATFORM_ANDROID)
		// UT99_ANDROID_V104_UWEB_NATIVEFUNC_GUARD:
		// UWeb may still be loaded by old saved configs or mods even though WebAdmin
		// is disabled on Android. Do not fatal in GetDllExport(..., Checked=1) for
		// its dynamic native functions; bind them to execUndefined as a harmless
		// fallback. The server-actor path is skipped separately in UnLevel.cpp.
		if( ClassPackage && ClassPackage->GetFName()==FName(TEXT("UWeb")) )
		{
			Native* Ptr = (Native*)ClassPackage->GetDllExport( Proc, 0 );
			Func = Ptr ? *Ptr : &UObject::execUndefined;
		}
		else
#endif
		{
