...
class ULinkerLoad : public ULinker, public FArchive
{
	...
	,	LoadFlags( InLoadFlags )
	{
		...
		DOUBLE UT99LinkerStart = UT99LoadProfSeconds();
		DOUBLE UT99StageStart = UT99LinkerStart;
		...
		UT99StageStart = UT99LoadProfSeconds();
		...
		*this << Summary;
		...
		// Load and map names.
		DOUBLE UT99TablesStart = UT99LoadProfSeconds();
		guard(LoadNames);
		...
		Success = 1;
		{
			DOUBLE UT99LinkerMs = UT99_LOADPROF_MS(UT99LinkerStart);
			if( UT99LinkerMs >= 250.0 )
				UT99_LOADPROF_LOG("LINKER_END_SLOW package=\"%s\" file=\"%s\" names=%d imports=%d exports=%d total_ms=%.3f", LinkerRoot ? TCHAR_TO_ANSI(LinkerRoot->GetName()) : "<null>", TCHAR_TO_ANSI(*Filename), Summary.NameCount, Summary.ImportCount, Summary.ExportCount, UT99LinkerMs );
		}
	...
	void Verify()
	{
		...
		DOUBLE UT99VerifyStart = UT99LoadProfSeconds();
		if( !Verified )
		{
			if( Cast<UPackage>(LinkerRoot) )
		...
		if( !Verified )
		{
			DOUBLE UT99VerifyMs = UT99_LOADPROF_MS(UT99VerifyStart);
			if( UT99VerifyMs >= 150.0 )
				UT99_LOADPROF_LOG("VERIFY_SLOW package=\"%s\" imports=%d ms=%.3f", LinkerRoot ? TCHAR_TO_ANSI(LinkerRoot->GetName()) : "<null>", Summary.ImportCount, UT99VerifyMs );
		}
		Verified=1;
		unguard;
	}
	...
	void VerifyImport( INT i )
	{
		...
		if( Import.PackageIndex == 0 )
		{
			...
			try
			{
				DOUBLE UT99ImportLinkStart = UT99LoadProfSeconds();
				Import.SourceLinker = GetPackageLinker( TmpPkg, NULL, LOAD_Throw | (LoadFlags & LOAD_Propagate), NULL, NULL );
				DOUBLE UT99ImportLinkMs = UT99_LOADPROF_MS(UT99ImportLinkStart);
				if( UT99ImportLinkMs >= 150.0 )
					UT99_LOADPROF_LOG("IMPORT_LINKER_SLOW package=\"%s\" import=\"%s\" ms=%.3f", LinkerRoot ? TCHAR_TO_ANSI(LinkerRoot->GetName()) : "<null>", TCHAR_TO_ANSI(*Import.ObjectName), UT99ImportLinkMs );
			}
	...
	void Preload( UObject* Object )
	{
		...
		if( Object->GetLinker()==this )
		{
			...
			if( Object->GetFlags() & RF_NeedLoad )
			{
				...
				guard(LoadObject);
				DOUBLE UT99PreloadStart = UT99LoadProfSeconds();
				...
				{
					DOUBLE UT99PreloadMs = UT99_LOADPROF_MS(UT99PreloadStart);
					if( UT99PreloadMs >= 150.0 )
						UT99_LOADPROF_LOG("PRELOAD_SLOW package=\"%s\" object=\"%s\" index=%d size=%d ms=%.3f", LinkerRoot ? TCHAR_TO_ANSI(LinkerRoot->GetName()) : "<null>", TCHAR_TO_ANSI(*Export.ObjectName), Object->_LinkerIndex, Export.SerialSize, UT99PreloadMs );
				}
