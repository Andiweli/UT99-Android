...
class SDLGenericMotionListener_API12 implements View.OnGenericMotionListener {
    ...
    protected float ut99V116ScaleMouseX(float x) {
        // UT99_ANDROID_NATIVE_MOUSE_HOVER_SCALE_V116
        // SOURCE_MOUSE hover/move events come through SDLGenericMotionListener,
        // while button down/up comes through SDLSurface.onTouch where V79 already
        // maps fullscreen view coordinates to the smaller 75%/50% SDL surface.
        // Scale hover/motion here too, otherwise the visible menu cursor and the
        // click/hit-test coordinates use different systems.
        try {
            SDLSurface surface = SDLActivity.mSurface;
            if (surface != null
                    && surface.mWidth > 1.0f && surface.mHeight > 1.0f
                    && surface.mUT99TouchWidth > 1.0f && surface.mUT99TouchHeight > 1.0f
                    && (surface.mUT99TouchWidth > surface.mWidth + 1.0f || surface.mUT99TouchHeight > surface.mHeight + 1.0f)) {
                x = x * (surface.mWidth / surface.mUT99TouchWidth);
                if (x < 0.0f) { x = 0.0f; }
                if (x > surface.mWidth - 1.0f) { x = surface.mWidth - 1.0f; }
            }
        } catch (Throwable ignored) {
        }
        return x;
    ...
    protected float ut99V116ScaleMouseY(float y) {
        // UT99_ANDROID_NATIVE_MOUSE_HOVER_SCALE_V116
        try {
            SDLSurface surface = SDLActivity.mSurface;
            if (surface != null
                    ...
                    && surface.mUT99TouchWidth > 1.0f && surface.mUT99TouchHeight > 1.0f
                    && (surface.mUT99TouchWidth > surface.mWidth + 1.0f || surface.mUT99TouchHeight > surface.mHeight + 1.0f)) {
                y = y * (surface.mHeight / surface.mUT99TouchHeight);
                if (y < 0.0f) { y = 0.0f; }
                if (y > surface.mHeight - 1.0f) { y = surface.mHeight - 1.0f; }
            }
        } catch (Throwable ignored) {
        }
        return y;
