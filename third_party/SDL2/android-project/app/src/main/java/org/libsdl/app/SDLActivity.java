...
*/
    ...
    public static boolean ut99DispatchImeTextToGame(String text) {
        // UT99_ANDROID_V82_IME_COMMIT_BRIDGE:
        // The stock SDL path can show DummyEdit without producing SDL_TEXTINPUT
        // on some devices/keyboards.  Keep this completely optional and reflective
        // so SDLActivity remains reusable outside the UT99 package.
        if (text == null || text.length() == 0) {
            return false;
        }
        try {
            Class<?> gameActivity = Class.forName("com.ast.ut99.GameActivity");
            java.lang.reflect.Method method = gameActivity.getMethod("ut99CommitImeText", String.class);
...
class SDLInputConnection extends BaseInputConnection {
    ...
    protected void updateText() {
        ...
        if (matchLength < text.length()) {
            ...
            // UT99_ANDROID_V82_IME_COMMIT_BRIDGE:
            // If UT99 currently owns an editable UWindow field, send committed
            // IME text through its native KeyType bridge and do not also feed it
            // to SDL, otherwise some keyboards duplicate characters.
            if (SDLActivity.ut99DispatchImeTextToGame(pendingText)) {
                mCommittedText = text;
                return;
            }
